...
\textbf{Build result} (Lean~4.28.0 + Mathlib4, run locally via \texttt{lake build}):
\begin{verbatim}
Build completed successfully (3103 jobs).
warning: unused variable `F`          -- lint note, harmless
warning: declaration uses `sorry`     -- only no_C2_potential_vcml (Clairaut step)
\end{verbatim}
\textbf{Zero errors.} All lemmas L1--L4 typecheck; the single \texttt{sorry}
is confined to the Clairaut step in \texttt{no\_C2\_potential\_vcml}.
The key verified theorems:

\begin{lstlisting}[language=Lean4, caption={Lean 4 proof of Lemmas L1--L4 (from \texttt{paper33\_curl\_proof.lean}, verified by \texttt{lake build})}]
-- L1: System definitions
noncomputable def f_F (fa F m : R) : R := fa * (m - F)
noncomputable def f_m (gamma F m : R) : R := -(1 - gamma) * m

-- L2a: df_F/dm = fa  (VERIFIED, no sorry)
theorem hasDerivAt_fF_wrt_m (fa F m : R) :
    HasDerivAt (f_F fa F) fa m := by
  unfold f_F
  have heq : (fun x : R => fa * (x - F)) = (fun x => fa * x - fa * F) := by
    funext x; ring
  rw [heq]
  have h := ((hasDerivAt_id m).const_mul fa).sub_const (fa * F)
  simpa [mul_one] using h

-- L2b: df_m/dF = 0  (VERIFIED, no sorry)
theorem hasDerivAt_fm_wrt_F (gamma F m : R) :
    HasDerivAt (fun F' => f_m gamma F' m) 0 F := by
  unfold f_m; exact hasDerivAt_const F _

-- L3: curl != 0  (VERIFIED, no sorry)
theorem vcml_curl_ne_zero (hfa : (0 : R) < fa) : (-fa : R) /= 0 := by linarith

-- L4: Full equations df_F_full/dm = fa  (VERIFIED, no sorry)
theorem hasDerivAt_fF_full_wrt_m (fa FD kappa F m : R) :
    HasDerivAt (f_F_full fa FD kappa F) fa m := by
  unfold f_F_full
  have heq : (fun m' : R => fa*(m'-F) - (1-FD)*F + kappa*(0-F)) =
             (fun m' => fa*m' + (-(fa*F) - (1-FD)*F + kappa*(0-F))) := by
    funext x; ring
  rw [heq]
  have h := ((hasDerivAt_id m).const_mul fa).add_const
              (-(fa*F) - (1-FD)*F + kappa*(0-F))
  simpa [mul_one] using h

-- Clairaut step: sorry (requires partialDeriv_comm from Mathlib)
theorem no_C2_potential_vcml (hfa : (0 : R) < fa) ... : False := by sorry
\end{lstlisting}

\paragraph{Verification status.}
Lemmas L1--L4 are fully machine-verified with \texttt{lake build}
(Lean~4.28.0, Mathlib4, 3103 compilation jobs, 0 errors).
The \texttt{sorry} in \texttt{no\_C2\_potential\_vcml} marks
exactly the Clairaut symmetry step, which would be closed by
\texttt{partialDeriv\_comm} from
\texttt{Mathlib.Analysis.Calculus.FDeriv.Symmetric}.
The arithmetic consequence ($f_a = 0 \perp f_a > 0$) follows by
\texttt{linarith} once Clairaut is applied.
